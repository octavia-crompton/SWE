...
************************************************************************
      include 'dry.inc'  
!       real*8  flux1, flux2, flux3, flux4
!       real*8  vol, vol0, dvol, flux, zinfl
      open(2,file ='coords')
...
C       calculate Red  (element reynolds number)  = UD/nu
!         Rdx = abs(udum*)
C         calculate Cd/per element
C     calculate effective xn = (cd*m*frontal area )*h^(1/3)/grav)^.5
...
C Fluxes and source terms.
      if(havg .le. 0.D0 ) then
        ...
        call solver(hl,hr,ul,ur,vl,vr,fdum,snn,cnn,dx,dy,sdumx,sdumy)
	      do i=1,3
          f(jr,kr,i,i1) = fdum(i)
        enddo
      endif
...
************************************************************************
      subroutine solver(hl,hr,ul,ur,vl,vr,f,sn,cn,dx,dy,sx,sy)
