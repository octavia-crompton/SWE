...
C           do l=1,3
C             q(j,k,l) = q(j,k,l) + dt/area(j,k)*(
C      &              (qsource(j,k,l,2) + f(j,k,l,2))*ds(j,k,2) +
C      &              (qsource(j,k,l,1) + f(j,k,l,1))*ds(j,k,1) +
C      &              (qsource(j+1,k,l,1) - f(j+1,k,l,1))*ds(j+1,k,1) +
C      &              (qsource(j,k+1,l,2) - f(j,k+1,l,2))*ds(j,k+1,2))
C           enddo
C           enddo
C           enddo
C         else
...
c	    call object
 ...
 201  format(' ','time is ',f8.1,' maximum CFL number is ',f7.2 )
      end
************************************************************************
      subroutine myoutput
      include 'dry.inc'
...
c      iflag = 0
C       if(ilim .eq. 0)then
C         sdumx = 0.5D0*(sx(jl,kl) + sx(jr,kr))
C         sdumy = 0.5D0*(sy(jl,kl) + sy(jr,kr))
C         if(hp(jl,kl) .ge. epsh .and. hp(jr,kr) .lt. epsh) then
C            if(etal .le. zc(jr,kr)) iflag = 1
C         endif
C         if(hp(jr,kr) .ge. epsh .and. hp(jl,kl) .lt. epsh) then
C           if(etar .le. zc(jl,kl)) iflag = 1
C         endif
C C Bed  infiltration, bottom drag.
C         if(havg .lt. epsh) then
C           qsource(jr,kr,1,i1) = 0.d0
C           qsource(jr,kr,2,i1) = -0.5D0*(grav*havg*sdumx)*dx*cnn
C           qsource(jr,kr,3,i1) = -0.5D0*(grav*havg*sdumy)*dy*snn
C         else
C           tavg = 0.5d0*(t0(jl,kl) + t0(jr,kr))
C           topp = t - tavg
C           if(topp .gt. 0.D0) then
C             if(topp .le. tc) then
C               zinflt = xk*topp**ainflt
C             else
C               zinflt = binflt
C             endif
C           else
C           zinflt = 0.D0
C         endif
C         vmag = dsqrt(uavg*uavg + vavg*vavg)
C         fricx = grav*xn*xn*uavg*vmag/havg**(1.D0/3.D0)
C         fricy = grav*xn*xn*vavg*vmag/havg**(1.D0/3.D0)
C         qsource(jr,kr,1,i1) = -0.25D0*winflt*(cnn*dx + snn*dy)
C         qsource(jr,kr,2,i1) = 0.5D0*(0.5D0*uavg*winflt - fricx -
C      &                             grav*havg*sdumx)*dx*cnn
C         qsource(jr,kr,3,i1) = 0.5D0*(0.5D0*vavg*winflt - fricy -
C      &                             grav*havg*sdumy)*dy*snn
C         endif
C       endif
...
C       if(sw .gt. 0.D0) then
C         if(hhat .ge. epsh) then
C           vmag = dsqrt(uhat*uhat + vhat*vhat)
C           fricx = grav*xn*xn*uhat*vmag/hhat**(1.D0/3.D0)
C           fricy = grav*xn*xn*vhat*vmag/hhat**(1.D0/3.D0)
C         else
C           fricx = 0.D0
C           fricy = 0.D0
C         endif
C         q1 = -winflt
C         q2 = -0.5D0*cn*(chat*chat*sx + fricx - 0.5D0*uhat*winflt)*dx*cn
C         q3 = -0.5D0*sn*(chat*chat*sy + fricy - 0.5D0*vhat*winflt)*dy*sn
C         z(1) = dabs(a(1))/a(1)*(a(3)*q1 - cn*q2 - sn*q3)/(2.D0*chat)
C         if(dabs(sn*a(2)) .gt. 1.D-8) then
C           z(2) = dabs(a(2))/a(2)*((uhat-uperp*cn)*q1 +
C      &                           sn*sn*q2 - sn*cn*q3)/sn
C         else
C           z(2) = 0.D0
C         endif
C         z(3)=dabs(a(3))/a(3)*(-a(1)*q1 + cn*q2 + sn*q3)/(2.D0*chat)
C       endif
C Entropy fix.
...
c        dt = tmax/dfloat(nt)
	  ...
	  do i=1,inum(j,k)
	    ...
	    if(hdum .le. 0.D0) then
	     write(*,*) 'ERROR: depth specification for solitary wave'
	     write(*,*) '       BC is invalid'
	     stop
	    endif
	   endif
	    ...
	    if(ipos(j,k,i) .eq. 2 .or. ipos(j,k,i) .eq. 4) then
	     ...
	     dss = dsqrt(dx*dx + dy*dy)
	      ...
	      if(qflux*dz(j,k,1) .lt. 0.D0 .and. xn .gt. 0.d0) then
               qflux = dabs(qflux)
	         hnorm=(qflux*xn/dsqrt(dabs(dz(j,k,1)/dss)))**(3.D0/5.D0)
	    ...
	    elseif(ipos(j,k,i) .eq. 1 .or. ipos(j,k,i) .eq. 3) then
	     ...
	     dss = dsqrt(dx*dx + dy*dy)
	      ...
	      if(qflux*dz(j,k,2) .lt. 0.D0 .and. xn .gt. 0.d0) then
              qflux = dabs(qflux)
	        hnorm=(qflux*xn/dsqrt(dabs(dz(j,k,2)/dss)))**(3.D0/5.D0)
	        write(*,*) ' *** normal depth = ', hnorm,' is specified'
              h(jj,kk) = hnorm 
              hp(jj,kk) = hnorm    
	       else
...
c	 enddo
C Compute interpolation distances.
c10     d1 = dsqrt((xdum - xc(j1(i),k1(i)))**2 + 
