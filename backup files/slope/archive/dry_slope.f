...
C   
C	    t0[j,k] = time when wetting occurs at cell [j,k].  initially zero, confusingly
C     it  = iteration number  (how many times through the time loop)
C     itp = print number, increment each time we print, should equal it/nprt  
C 
************************************************************************
      include 'dry_slope.inc'
      real vol, infl,flux1, flux2, flux3, flux4
      open(2,file ='coords_slope')
      open(3,file ='dryin_slope.dat')
      open(7,file ='object.dat')
      open(8,file ='dryout.dat')
      open(9,file ='movie.dat')
      open(11,file='restart.out')
	    open(13,file='slice.out')
	    open(101,file='time.out')               ! t, print step, time step, CFL
	    open(102,file='mycoords.out') 
	    open(103,file='gridmetrics.out')            
C       open(104,file='fluxes.out')           ! loop in myoutput can write top boundary fluxes to file
      open(105,file='test_point_slope.out')
      
C Read input, set up grid, initial conditions.
      call cpu_time(start)  
      call input
      if (impermeable .eq. 1) then
        open(100,file='myout_slope_imperm.out')        ! j,k,t0(j,k), h(j,k) every nprt time steps
        open(12,file='diag_imperm.out')                ! volume tracking
      else
        open(100,file='myout_slope.out')    
        open(12,file='diag.out')             
      endif
      infl = 0.d0
      flux1 = 0.d0
      flux2 = 0.d0
      ...
      write(12, *)  't, vol, infl, flux1, flux2, flux3, flux4'
	    write(12,200) t,vol, infl,flux1,flux2,flux3,flux4
      iprt = 0 
	    itp  = 0          
      call interp
	    call myoutput 
...
C Begin time loop.
      do while (t.lt.tmax)
	      it=it+1				        	  ! iteration count
C         if(t.gt.60.)   dt=0.2      ! possible problem solver.
C         if(t.gt.6960.) dt=1.0
c save every step for a movie      ! really?  
c        call movie
        t = t + dt
        amax = 0.D0
C Compute predictor.
...
C First order corrector solution
        if(ilim .eq. 0) then
          do j=1,ncol
            do k=kbeg(j),kend(j)
              do l=1,3
                    q(j,k,l) = q(j,k,l) + dt/area(j,k)*(
     &              (qsource(j,k,l,2) + f(j,k,l,2))*ds(j,k,2) +
     &              (qsource(j,k,l,1) + f(j,k,l,1))*ds(j,k,1) +
     &              (qsource(j+1,k,l,1) - f(j+1,k,l,1))*ds(j+1,k,1) +
     &              (qsource(j,k+1,l,2) - f(j,k+1,l,2))*ds(j,k+1,2))
       		          write(*,*) "do you know that ilim = 0?  "
		          enddo	  
            enddo
          enddo
C Second order corrector solution          
        else
          do j=1,ncol
            do k=kbeg(j),kend(j)
              call source(j,k,hp(j,k),up(j,k),vp(j,k))
              infl = infl + qs(1)*dt*area(j,k)
              do l=1,3
                q(j,k,l) = q(j,k,l) + dt/area(j,k)*(   
...
C           do j=0,ncol+1   ! is infiltration in the ghost cells an issue?  
C             do k=kbeg(j)-1,kend(j)+1
C               call source(j,k,hp(j,k),up(j,k),vp(j,k))
C               infl0 = infl0 + qs(1)*dt*area(j,k)
C             enddo
C           enddo 
        endif
C Store solution. 
...
C Neglect momentum in nearly dry cells.
            if(h(j,k) .lt. epsh) then  
              u(j,k) = 0.d0
              v(j,k) = 0.d0
              t0(j,k) = t  							 ! t0 = t in nearly dry cells
              do l=2,3
                q(j,k,l) = 0.d0
              enddo
          ...
          enddo
        enddo 
C   Done storing solution
C   Below here only executed every nprt time steps        
        iprt = iprt + 1                   ! increment print step
	      if(iprt .eq. nprt) then
	        iprt = 0
		      itp = itp + 1
          call myoutput
C Check volume error.
          if(imass .eq. 1) then
            vol = 0.D0
            do j=1,ncol
              do k=kbeg(j),kend(j)
            ...
            do j=0,ncol+1
              do k=kbeg(j)+1,kend(j)+1
                vol0 = vol0 + h(j,k)*area(j,k)
            enddo
          enddo
        endif
C Find location of wetting front for plotting.
          if(ifront .eq. 1) then
            xf = 0.d0
	          do j=1,ncol
              do k=kbeg(j),kend(j)
                n2 = nop(j,k,2)
                n3 = nop(j,k,3)
                xdum = dmax1(x(n2),x(n3))
                if(h(j,k) .ge. epsh .and. xdum .gt. xf)  then
                  xf = xdum
                endif
              enddo
            enddo 
...
C           do i=1,nmon
C             call sample(0,i,xmon(i),ymon(i),hn(i),un(i),vn(i))
C           enddo
           write(12,200)  t,vol, infl, flux1, flux2, flux3, flux4
           write(*,201)    t, amax*dt

        endif    
C         if(amax*dt>10) then
C           write(*,*) 'CFL too big'
C            stop
C          endif
      enddo
...
c 	call object
 ...
 200  format(' ',9e15.6)
C  201  format(' ','time is ',e12.4, ' maximum CFL number is ',f7.2 )
 201  format(' ','time is ', f7.1 , ' maximum CFL number is ',f10.3 )
      end
...
************************************************************************
      subroutine myoutput
      include 'dry_slope.inc'
C     call every nprt time steps      
C 	  file 101 is 'time.out'  -  to keep track of the time stpes    
      if (t .eq. 0) then
         write(101, *) "time  |  print step  |  time step | CFL"
       endif
      write(101, *) t, itp, it, amax*dt

C 	  file 100 is 'myout.out' 	    
      write(100, 202)  itp
      do j=1,ncol
        do k=kbeg(j),kend(j)
          write(100, *) j, k, t0(j,k), h(j,k)
        enddo
      enddo     
...
C             do i=1,inum(j,k)
C               if( ipos(j,k,i) .eq. 3) then  ! horizontal boundaries
C                   write(104, *) j,k, f(j,k+1,1, 2)*ds(j, k+1, 2)
C               endif
C             enddo
C           enddo
C         enddo
  
C 	  file 105 is 'test_point_slope.out' 	   
C     record h, u, v at test point (j = 43, k = 61)  
        j = 43
        k = 61     
        write(105, *)  h(j, k), u(j, k), v(j, k)
        
                
      return
 200  format(' ', i6, i6, 5e15.7, 5e15.7)
 202  format(' ', 4i6)	 
 203  format(' ',f7.4 , 4i6, 4i6 ) 
      end			
...
C Prevent leakage into cells with higher bed elevation.
      ...
      iflag = 0
      if(ilim .eq. 0)then
	    sdumx = 0.5D0*(sx(jl,kl) + sx(jr,kr))
	    sdumy = 0.5D0*(sy(jl,kl) + sy(jr,kr))
        if(hp(jl,kl) .ge. epsh .and. hp(jr,kr) .lt. epsh) then
          if(etal .le. zc(jr,kr)) iflag = 1
        endif
        if(hp(jr,kr) .ge. epsh .and. hp(jl,kl) .lt. epsh) then
          if(etar .le. zc(jl,kl)) iflag = 1
        endif
C Bed  infiltration, bottom drag.
        if(havg .lt. epsh) then
          qsource(jr,kr,1,i1) = 0.d0
	        qsource(jr,kr,2,i1) = -0.5D0*(grav*havg*sdumx)*dx*cnn
          qsource(jr,kr,3,i1) = -0.5D0*(grav*havg*sdumy)*dy*snn
        else 
          tavg = 0.5d0*(t0(jl,kl) + t0(jr,kr)) 
	      topp = t - tavg
	      if(topp .gt. 0.D0) then
            if(topp .le. tc) then
          ...
          vmag = dsqrt(uavg*uavg + vavg*vavg)
          fricx = grav*xn*xn*uavg*vmag/havg**(1.D0/3.D0)
          fricy = grav*xn*xn*vavg*vmag/havg**(1.D0/3.D0)
          qsource(jr,kr,1,i1) = -0.25D0*winflt*(cnn*dx + snn*dy)
	        qsource(jr,kr,2,i1) = 0.5D0*(0.5D0*uavg*winflt - fricx - 
     &                             grav*havg*sdumx)*dx*cnn
          qsource(jr,kr,3,i1) = 0.5D0*(0.5D0*vavg*winflt - fricy -
     &                             grav*havg*sdumy)*dy*snn
        endif
      endif
C Fluxes and source terms.
      if(havg .le. 0.D0 .or. iflag .eq. 1) then
	      do i=1,3  
          f(jr,kr,i,i1) = 0.D0
          qsource(jr,kr,i,i1) = 0.d0
	      enddo
      else 
...
C Compute right eigenvectors.
      ...
      e(2,3) = uhat + chat*cn
      e(3,3) = vhat + chat*sn
C Compute approximate source terms for upwind method only.   
      if(sw .gt. 0.D0) then
        if(hhat .ge. epsh) then
	        vmag = dsqrt(uhat*uhat + vhat*vhat)
	        fricx = grav*xn*xn*uhat*vmag/hhat**(1.D0/3.D0)
	        fricy = grav*xn*xn*vhat*vmag/hhat**(1.D0/3.D0)
	      else
	        fricx = 0.D0
          fricy = 0.D0
	      endif
        q1 = -winflt
        q2 = -0.5D0*cn*(chat*chat*sx + fricx - 0.5D0*uhat*winflt)*dx*cn
        q3 = -0.5D0*sn*(chat*chat*sy + fricy - 0.5D0*vhat*winflt)*dy*sn
        z(1) = dabs(a(1))/a(1)*(a(3)*q1 - cn*q2 - sn*q3)/(2.D0*chat)
	      if(dabs(sn*a(2)) .gt. 1.D-8) then
          z(2) = dabs(a(2))/a(2)*((uhat-uperp*cn)*q1 + 
     &                           sn*sn*q2 - sn*cn*q3)/sn
	      else
          z(2) = 0.D0
	      endif
          z(3)=dabs(a(3))/a(3)*(-a(1)*q1 + cn*q2 + sn*q3)/(2.D0*chat)
	    endif     ! omit
C Entropy fix.
...
C         if (h(j,k) .gt. epsh) write(*, *)  j, k, qs(1) , '!!!!!'
...
C             write(*, *) fix(j,k,3),hdum(jj,kk), vdum(jj,kk)
	        else
            udum(jj,kk) = 0.D0
            vdum(jj,kk) = 0.D0
          endif 
C           write(*,*) hdum(jj,kk), udum(jj,kk), vdum(jj,kk)
...
***********************************************************************
      subroutine grid
      include 'dry_slope.inc'
C Read grid data from file 'coords' (2). 
      read(2,*) np, ne
      if(np .gt. nn) then
      ...
      do j=1,ncol
        ...
        enddo
      enddo
C Compute grid metrics.zn = np.zeros(npt+1)
      do j=1,ncol
        do k=kbeg(j),kend(j)
          n1 = nop(j,k,1)
          n2 = nop(j,k,2)
...
C Read input from file 'dry.dat'.
      ...
      read(3,'(a72)') dum
      read(3,*) impermeable
      write(*, *) impermeablewitwr
      if(tmax .gt. 500) nprt = 30
      if(tmax .gt. 1000) nprt = 50
      read(3,'(a72)') dum
      read(3,*) nbcell
      read(3,'(a72)') dum
      do ii=1,nbcell
...
C       read(3,'(a72)') dum
C       do i=1,nmon
C        read(3,*) idum, xmon(idum), ymon(idum)
C       enddo
C       read(3,'(a72)') dum
C     Track u, v, h at stations
C      do i=  1,nmon
C         read(3,'(a72)') filename(i)
C         idev = 13 + i
C         open(idev,file=filename(i))
C       enddo
...
C       read (*,*) ilim
	  ilim = 5
C Upwind the source terms if upwind differenceing is requested.
      sw = 0.d0
      if(ilim .eq. 0) sw = 1.d0
      write(*,*) ' '  
C Read from restart file.
      if(istart .eq. 1)then
        write(*,*) '    enter restart time'
        read(*,*) t
c        dt = (tmax - t)/dfloat(nt)
        do j=1,ncol
        do k=kbeg(j),kend(j)
         read(11,*) jdum, kdum, h(j,k), u(j,k), v(j,k), t0(j,k) 
        enddo
        enddo 
        rewind 11
	endif
...
C Set initial conditions.
      if(istart .ne. 1) then   ! not reading from the restart file!
        t = 0.D0  			   ! set the time to zero
c        dt = tmax/dfloat(nt)
        do j=1,ncol
          do k=kbeg(j),kend(j)
C            n1 = nop(j,k,1)
...
C            n4 = nop(j,k,4)
           if(xc(j,k) .le. xsplit)then  
	         ...
	         if(xk .gt. 0.D0 .and. h(j,k) .eq. 0.D0) t0(j,k) = t   ! seems unnecessary
          enddo
        enddo     
      endif 
      do j=1,ncol
        do k=kbeg(j),kend(j)
          ...
          q(j,k,3) = h(j,k)*v(j,k)
          do i=1,inum(j,k)
C   itype 5:  solitary wave
            if(itype(j,k,i) .eq. 5 .and. fix(j,k,1) .ne. 0.D0) then
	            if(isurf .eq. 1) then
                hdum = fix(j,k,1) - zc(j,k)
	            else
	              hdum = fix(j,k,1)
              endif
	            if(hdum .le. 0.D0) then
	              write(*,*) 'ERROR: depth specification for solitary'
                write(*,*) '       wave BC is invalid'
	            stop
	            endif
            endif   ! end itype 5
C   itype 4: for fixed flux BCs, must initialize depth.
	          if(itype(j,k,i) .eq. 4) then
              write(*,*) ' '
              write(*,*) 'fixed flux BC is requested in cell ',j,k
              call findbc(i,j,k,jj,kk,j2,k2)
	            if(ipos(j,k,i) .eq. 2 .or. ipos(j,k,i) .eq. 4) then
                qflux = fix(j,k,2)*cn(j,k,1) + fix(j,k,3)*sn(j,k,1)
                dx = deta(j,k,2)*area(j,k)
                dy = -deta(j,k,1)*area(j,k)
                dss = dsqrt(dx*dx + dy*dy)
                if(h(j,k) .lt. epsh) then
                  write(*,*) ' bed adjacent to the boundary is dry'
                  if(qflux*dz(j,k,1) .lt. 0.D0 .and. xn .gt. 0.d0) then
                    qflux = dabs(qflux)
                    hnorm=(qflux*xn/dsqrt(dabs(dz(j,k,1)/dss)))
     &                  **(3.D0/5.D0)
                    write(*,*) ' normal depth = ', hnorm,' is specified' 
                    h(jj,kk) = hnorm 
                    hp(jj,kk) = hnorm   
         	        else
                ...
                qflux = dabs(qflux)
                tclip1 = (xk*dss/(qflux*(1.d0-cappa)))
     &                    **(1.d0/(1.d0-ainflt))
                if(tclip1 .gt. tclip) tclip = tclip1
              elseif(ipos(j,k,i) .eq. 1 .or. ipos(j,k,i) .eq. 3) then
...
C                   read(*,*) h(jj,kk)
     ...
     &                      (1.d0/(1.d0-ainflt))
            ...
            endif    ! end fixed flux BC
C For specified free surface BCs.
	          if(itype(j,k,i) .eq. 3 .and. isurf .eq. 1) then
	            check = fix(j,k,1) - zc(j,k) - amp
              if(check .lt. 0.D0) then
	              write(*,*) 'WARNING: specified tidal amplitude will'
                write(*,*) 'result in negative specified depth'
                write(*,*) 'specified depth will be set to zero'
              endif
            endif
          enddo
        enddo
...
C For problems with bed seepage.
      if(xk .gt. 0.d0) then
        if(tclip .gt. 0.d0) then
          ...
          tclip = 0.d0   			! seems not necessary
        endif
      endif
      itest = 0
      write(*,*) ' '
...
C         write(102,*) i, j1(i), k1(i),d1,d2,d3,d4
...
C Write to file 'restart.out', 11
      do j=1,ncol
        do k=kbeg(j),kend(j)
          write(11,199) j, k, h(j,k), u(j,k), v(j,k), t0(j,k)
        enddo
      enddo
C Write to file 'mycoords.out', 102          
      do k=0,64
        write(102, *) 0, k, xc(0,k), yc(0,k), zc(0,k)
      enddo
      do j=1,ncol+1
        do k=kbeg(j)-1,kend(j)+1
          write(102, *) j, k, xc(j,k), yc(j,k), zc(j,k)
        enddo
      enddo

C Write to file 'gridmetrics', 103
      do j=1,ncol
        do k=kbeg(j),kend(j)
          write(103, *) j, k, area(j,k), q(j,k, 1), q(j, k, 2),
     &          q(j, k, 3)
        enddo
      enddo         
      
C Write to file 'dryout.out'.
C fr = froude number.  defined as the ratio of the flow inertia to the external field (gravity)
      write(*,*) ' '
C       write(*,*) 'OUTPUT OPTIONS:'
C       write(*,*) 'enter 1 to plot depth'
C       write(*,*) '      2 to plot bed'
C       write(*,*) '      3 to plot free surface'
C       read(*,*) iout
      iout = 3
	    if(iout .eq. 1) then
C         write(8,*) 'VARIABLES = "X", "Y", "H", "U", "V", "Z", "Fr"'
C         write(8,*)'ZONE N=',np,' E=',ne,' F=FEPOINT',' ET=QUADRILATERAL'
        do n=1,np
          if(hn(n) .gt. 0.D0) then   
            fr = dsqrt((un(n)*un(n) + vn(n)*vn(n))/(grav*hn(n)))  ! fr = u/sqrt(g*l)
          else
            fr = 0.D0
          endif
...
C         write(8,*) 'VARIABLES = "X", "Y", "Z", "U", "V", "H", "Fr"'
C         write(8,*)'ZONE N=',np,' E=',ne,' F=FEPOINT',' ET=QUADRILATERAL'
        do n=1,np
          if(hn(n) .gt. 0.D0) then
...
C         write(8,*) 'VARIABLES = "X", "Y", "H+Z", "U", "V", "H", "Z", "Fr"'
C         write(8,*)'ZONE N= ',np,' E=',ne,' F=FEPOINT', ' ET=QUADRILATERAL'
      do n=1,np
          if(hn(n) .gt. 0.D0) then
            fr = dsqrt((un(n)*un(n) + vn(n)*vn(n))/(grav*hn(n)))
      ...
      do j=1,ncol
        do k=kbeg(j),kend(j)
	      ...
	      enddo
	    enddo
C       write(*,*) 'enter a 1 to extract a slice of output'
C       read(*,*) iext
      iext = 0
      if(iext .eq. 1) then
     ...
     &          fr = dsqrt(un(n)**2+vn(n)**2)/dsqrt(grav*hn(n))
          ...
          enddo
        else   ! if islice  = 1 (for a constant y slice)
...
C 199  format(' ',2i6,4e15.7)
C  199  format(' ',2i6,4e15.7)
 200  format(' ',f12.3,f12.3, 9e15.5)
 201  format(' ',4i6)
      end
************************************************************************
      subroutine movie
      include 'dry_slope.inc'
      if(itest.eq.0) then
        write(*, *) "computing weights!  check for error!"
        do i=1,np
          xdum=x(i)
          ydum=y(i)
C Bracket sampling location.
...
C Write to file 'movie.dat'.
      if(t.eq.0.)then
        ...
        write(9,*) 'VARIABLES = "X", "Y", "Z", "H", "D"'
        write(9,*) 'ZONE N=',np,'E=',ne,' F=FEPOINT',
     &           ' ET=QUADRILATERAL', ' D=(FECONNECT,1,2,3)'        
  	  	  do n=1,np
...
C Bracket sampling location.
c       do j=1,ncol
c       do k=kbeg(j),kend(j)+1
c     if(xdum .ge. xc(j-1,k) .and. xdum .lt. xc(j,k)) then
c       if(ydum .ge. yc(j,k-1) .and. ydum .le. yc(j,k)) then
c        j1(i) = j
c        k1(i) = k
c        goto 10
c       endif
c      elseif(xdum .ge. xc(j,k) .and. xdum .le. xc(j+1,k)) then
c       if(ydum .ge. yc(j,k-1) .and. ydum .le. yc(j,k)) then
c        j1(i) = j+1
c        k1(i) = k
c        goto 10
...
c   enddo
C Compute interpolation distances.
c 10     d1 = dsqrt((xdum - xc(j1(i),k1(i)))**2 +
c  &            (ydum - yc(j1(i),k1(i)))**2)
c      d2 = dsqrt((xdum - xc(j1(i)-1,k1(i)))**2 +
c  &            (ydum - yc(j1(i)-1,k1(i)))**2)
c       d3 = dsqrt((xdum - xc(j1(i)-1,k1(i)-1))**2 +
c  &            (ydum - yc(j1(i)-1,k1(i)-1))**2)
c       d4 = dsqrt((xdum - xc(j1(i),k1(i)-1))**2 +
c  &            (ydum - yc(j1(i),k1(i)-1))**2)
C Compute weights as distance inverses.
C Tecplot states that the 3.5 exponent yields the smoothest results.
c    if(d1 .gt. 0.D0) then
c      w1(i) = d1**(-3.5D0)
c     else
c      w1(i) = 1.D0
c      w2(i) = 0.D0
...
c      goto 4
c    endif
c 4    sumw = w1(i) + w2(i) + w3(i) + w4(i)
c
C Interpolate data.
c    sumw = w1(i) + w2(i) + w3(i) + w4(i)
c  hn(i) = (w1(i)*h(j1(i),k1(i)) + w2(i)*h(j1(i)-1,k1(i)) +
c     &        w3(i)*h(j1(i)-1,k1(i)-1) + w4(i)*h(j1(i),k1(i)-1))/sumw
c  z(i) = (w1(i)*zc(j1(i),k1(i)) + w2(i)*zc(j1(i)-1,k1(i)) +
c     &        w3(i)*zc(j1(i)-1,k1(i)-1) + w4(i)*zc(j1(i),k1(i)-1))/sumw
c  enddo
C Write to file 'object.dat'.
c      new=0
c  do j=1,ncol
c        do k=kbeg(j),kend(j)
c      hbar=0.
...
c        hbar=hbar+hn(n)
c      enddo
c      if(hbar.gt.0.0) then
c        do i=1,4
c          n=nop(j,k,i)
c      enddo
c
c      endif
c      if(ndry(j,k).eq.0)new=new+1
c        enddo
c      enddo
c  npw=0
c  do i=1,np
c    if(hn(i).gt.0.0) npw=npw+1
c  enddo
c
c  write(7,*) 'VARIABLES = "X", "Y", "Z"'
c      write(7,*)'ZONE N= ',npw,' E=',new,' F=FEPOINT',' ET=BRICK'
c
c  do n=1,np
c    if(hn(n).gt.0.0)then
c         write(7,200) x(n),y(n),hn(n)+z(n)
c        write(7,200) x(n),y(n),z(n)
c    endif
c  enddo
c  do j=1,ncol
c        do k=kbeg(j),kend(j)
c          write(7,201) (nop(j,k,i), i=1,4),(nop(j,k,i)+ne, i=1,4)
c    enddo
c  enddo
      return
c 200  format(' ',3e15.5)
