...
C	  t0[j,k] = time when wetting occurs at cell [j,k]?
...
************************************************************************
	  ...
	  open(100,file='myout1.out')
	  open(101,file='time1.out')
...
C Find location of wetting front for plotting.
         ...
         do i=1,nmon
	     call sample(0,i,xmon(i),ymon(i),hn(i),un(i),vn(i))
         enddo
         itest = 1
...
C  201  format(' ','time is ',e12.4, ' maximum CFL number is ',f7.2 )
 201  format(' ','time is ',f7.2 , ' maximum CFL number is ',f7.2 )
      end
...
C            n4 = nop(j,k,4)
	     ...
	     if(xk .gt. 0.D0 .and. h(j,k) .eq. 0.D0) t0(j,k) = t   ! this line seems totally unnecessary
        enddo
        enddo     
      endif 
      do j=1,ncol
...
C For fixed flux BCs, must initialize depth.
     ...
     &			  				**(3.D0/5.D0)
              ...
              qflux = dabs(qflux)
            tclip1 = (xk*dss/(qflux*(1.d0-cappa)))**(1.d0/(1.d0-ainflt))
...
C			      h(jj,kk) = .4
              ...
              qflux = dabs(qflux)
            tclip2 = (xk*dss/(qflux*(1.d0-cappa)))**(1.d0/(1.d0-ainflt))
