...
C Check for negative depth.
            ...
            elseif(q(j,k,1) .lt. 0.D0) then
               write(*,*) 'negative q(1)= ', q(j,k,1)
            else 
              q(j,k,1) = 0.D0
              if (h(j,k).gt.0) write(*,*) 'degative h =', h
              h(j,k)   = 0.D0
            endif
...
C                 n2 = nop(j,k,2)
C                 n3 = nop(j,k,3)
C                 xdum = dmax1(x(n2),x(n3))
C                 if(h(j,k) .ge. epsh .and. xdum .gt. xf)  then
C                   xf = xdum
C                 endif
C               enddo
C             enddo
...
C           enddo
C            write(12,200)  t,vol, infl, flux1, flux2, flux3, flux4
           write(*,201)   t, amax*dt
