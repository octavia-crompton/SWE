...
C     itp = print number, increment each time we call myprint, should equal it/nprt  

C     'time.out' print t, itp, it, CFL each time we call myprint
C      boundarymiss : calculate infliltration using expanded boundaries
C      inflmiss: check for any lost volume when we set negative depth to zero
************************************************************************
      include 'dry_open.inc'
      real vol, infl,flux1, flux2, flux3, flux4
      real boundarymiss, inflmiss
      
      open(2,file ='coords_open')             !  coordinate file
      open(3,file ='dryin_open.dat')          !  boundaries file
      open(8,file ='dryout.dat')              !  output subroutine
      open(11,file='restart.out')             !  output subroutine
	    open(13,file='slice.out')
	    open(101,file='time.out')               !  t, itp, it, clf
	    open(102,file='mycoords.out') 
C     open(104,file='fluxes.out')            ! myprint:   write top boundary fluxes to file       
C     open(105,file='test_point.out')        ! myprint:  h, u, v at a test point 
...
C Read input, set up grid, initial conditions.      
      if (impermeable .eq. 1) then
        open(100,file='myout_open_imperm.out')        ! j,k,t0(j,k), h(j,k) every nprt time steps
        open(12,file='diag_imperm.out')               ! volume tracking
      else
        open(100,file='myout_open.out')    
        open(12,file='diag.out')    
        open(120,file='missing.out')                  ! missing infiltration?  
      endif 
      infl = 0.d0
      flux1 = 0.d0
      flux2 = 0.d0
      ...
      write(120, *)   't, boundarymiss, inflmiss'      
      write(120,200)  t,fluxmiss, inflmiss
      iprt = 0 
	    itp  = 0          
      call interp
	    call myprint 
...
C Second order corrector solution          
     ...
     &	 		   	  + dt*qs(l)
            enddo	
C             increment infiltration 
            infl = infl + qs(1)*dt*area(j,k) 
          enddo
        enddo
C          check whether infiltration in the ghost cells an issue?  
         do j=0,ncol+1  
          do k=kbeg(j)-1,kend(j)+1
            call source(j,k,hp(j,k),up(j,k),vp(j,k))
            fluxmiss = fluxmiss + qs(1)*dt*area(j,k)
          enddo
         enddo 
C Store solution. 
...
C Check for negative depth.
            if(q(j,k,1) .gt. 0.D0) then
              h(j,k) = q(j,k,1)
            elseif(q(j,k,1) .lt. 0.D0) then
               write(*,*) q(j,k,1)
C               inflmiss = inflmiss + q(j,k,1)*dt*area(j,k)
            else 
              q(j,k,1) = 0.D0
              if (h(j,k).gt.0) write(*,*) 'h =', h
              inflmiss = inflmiss + h(j,k)*dt*area(j,k)
              h(j,k)   = 0.D0
            endif
...
C 	  file 105 is 'test_point_open.out' 	   
C     record h, u, v at test point (j = 43, k = 61)  
C         j = 43
C         k = 61
C         write(105, *)  h(j, k), u(j, k), v(j, k)
...
C Set initial conditions.
      if(istart .ne. 1) then  ! no restart file
