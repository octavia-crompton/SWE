...
************************************************************************
      ...
      real  vol, vol0, dvol, flux, infl, rain
      open(2,file ='coords')
...
C Compute corrector solution.
        ...
        do k=kbeg(j),kend(j)
          ...
          infl = infl + dt*qs(1)*area(j,k) - prate*area(j,k)*dt
          do l=1,3
...
C         write(*,*) prate*dt
        do j=1,ncol
          do k=kbeg(j),kend(j)
            vol = vol + h(j,k)*area(j,k)
            rain = rain + prate*area(j,k)*dt
          enddo
        enddo 
        dvol = vol - vol0
        flux =  flux1 + flux2 + flux3 + flux4     
        write(120,200) t, dvol, flux, infl, rain        
...
C    Loop over cells to compute fluxes.  file 104 is 'fluxes.out'
      do j=1,ncol
        do k=kbeg(j),kend(j)
          do i=1,inum(j,k)
            if( ipos(j,k,i) .eq. 1) then   ! horizontal boundaries
...
C     Read input from file 'dryin.dat'.
      read(3,'(a72)') dum
