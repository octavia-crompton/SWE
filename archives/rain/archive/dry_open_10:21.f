...
************************************************************************
      ...
      open(2,file ='coords_open')
      open(3,file ='dryin_open.dat')
...
C Read input, set up grid, initial conditions.    
      ...
      if (impermeable .eq. 1) then
        open(100,file='myout_imperm.out')        ! j,k,t0(j,k), h(j,k) every nprt time steps
        open(12,file='diag_imperm.out')          ! volume tracking
        open(120,file='dvol_imperm.out')                 
      else
        open(100,file='myout.out')    
...
C       if(ifront .eq. 1 .or. imass .eq. 1) open(12,file='diag.out')
	    iprt = 0 
	    itp  = 0             
	    call interp
c      call myoutput      
...
C Begin time loop.
      do while (t.lt.tmax)
        ...
        if(t.gt.60.)   dt=0.1      ! possible problem solver.
C       if(t.gt.6960.) dt=0.5
...
C           enddo
          if(ifront .eq. 1 .or. imass .eq. 1) write(12,200) t,vol,xf
          write(*,201)  t, amax*dt
        endif    
C         if(amax*dt>10) stop
      enddo
C End of time loop.
      call movie
c 	call object
 ...
 200  format(' ',f10.1, 5e20.12)
C  201  format(' ','time is ',e12.4, ' maximum CFL number is ',f7.2 )
 201  format(' ','time is ', f8.1 , ' maximum CFL number is ',f10.3 )
      end

************************************************************************
      subroutine mycoords
      include 'dry.inc'
C 	  file 103 is 'mycoords.out' 
C       do j=1,ncol
C         do k=kbeg(j),kend(j)
C           write(102, *) j, k, xc(j,k), yc(j,k), zc(j,k)
C         enddo
C       enddo
C       return
...
C 	  file 100 is 'myout.out' 
      if (t .eq. 0) write(101, *) "time  |  print step  |  time step"
      write(101, *) t, itp, it
	    write(100, 202)  itp
      do j=1,ncol
        do k=kbeg(j),kend(j)
...
C                   write(104, *) j,k, f(j, k,1, 2)
                  write(104, *) j,k,f(j,k+1,1, 2), h(j, k), h(j, k+1)
              endif
            enddo
          enddo
        enddo
...
********************************s****************************************
      ...
      if(hdum .ge. epsh) then
        ...
        if (impermeable .gt. 0) winflt = 0.d0
        qs(1) = winflt
...
C Compute approximate source terms for upwind method only.
      if(sw .gt. 0.D0) then
        ...
        if(dabs(sn*a(2)) .gt. 1.D-8) then
          z(2) = dabs(a(2))/a(2)*((uhat-uperp*cn)*q1 +          ! poor choice variable name here?
...
C Write to file 'restart.out', 11
      do j=1,ncol
        ...
        enddo
      enddo
C Write to file 'mycoords.out', 11      
