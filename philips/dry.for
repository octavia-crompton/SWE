...
************************************************************************
      ...
      open(120,file= trim(folder)//'dvol.out')                                  
      call cpu_time(start)       
...
************************************************************************
      ...
      if(hdum .ge. epsh) then
        ...
        if(tnew .le. 0.d0) then
          znew = 0.d0
        elseif(tnew .gt. 0.d0 ) then
          znew = Soc(j,k)*(tnew**0.5d0) + satKc(j,k)*tnew
        endif
        told = dmax1(0.d0, t - t0(j,k) - dt)
	      if(told .le. 0.d0) then
          zold = 0.d0
        elseif(told .gt. 0.d0) then
          zold =  Soc(j,k)*(told**0.5d0) + satKc(j,k)*told
	      endif
        winflt = (zold - znew)/dt + prate
        if (winflt .lt. 0.d0) then
          winflt = 0.d0
          write(*,*) (zold - znew)/dt + prate
        endif
...
!         if(prate .gt. 0.d0) winflt = prate
        ...
        fricx = grav*xnc(j,k)*xnc(j,k)*udum*vmag/hdum**(1.D0/3.D0)
        fricy = grav*xnc(j,k)*xnc(j,k)*vdum*vmag/hdum**(1.D0/3.D0)
        qs(1) = winflt
...
C     Read grid data from file 'coords'.
      ...
      do i=1,np
        read(2,*) x(i), y(i), z(i), xn(i), satk(i), So(i)
      enddo
...
C Compute grid metrics.
      ...
      do k=kbeg(j),kend(j)
        ...
        satKc(j,k) = 0.25D0*(satk(n1) + satk(n2) + satk(n3) + satk(n4))
        Soc(j,k) = 0.25D0*(So(n1) + So(n2) + So(n3) + So(n4))
...
C     Read 199rom file 'dryin.dat'.
      read(3,'(a72)') dum
      read(3,*) grav, dt, tmax, xsplit
      read(3,'(a72)') dum
      read(3,*) epsh, beta, prate
!       prate =  prate/3600.d0
      read(3,'(a72)') dum
      read(3,*) xk, ainflt, binflt, tr, cappa
      read(3,'(a72)') dum
